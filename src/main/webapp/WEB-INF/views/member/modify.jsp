  ...
  <div class="box-body">
    ...
    <div class="form-group row">
      <label for="sample2_postcode" class="col-2">우편번호</label>
        <div class="col-8">
      <input type="email" class="form-control" name = "mbsp_zipcode" id="sample2_postcode" value="${memberVO.mbsp_zipcode }" placeholder="우편번호입력...">
        </div>
        <div class="col-2">
    ...
    <div class="form-group row">
      <label for="mbsp_phone" class="col-2">전화번호</label>
        <div class="col-10">
      <input type="text" class="form-control" oninput="oninputPhone(this)" maxlength="14" name = "mbsp_phone" id="mbsp_phone" value="${memberVO.mbsp_phone }" placeholder="전화번호입력...">
        </div>
    </div>
...
<%@include file="/WEB-INF/views/comm/plugin.jsp" %>
    ...
    // 별칭 : $ -> jQuery()함수
    // ready()이벤트 메소드 : 브라우저가 html태그를 모두 읽고난 후에 동작하는 이벤트 특징
    ...
    $(document).ready(function() {
      ...
      // document.getElementById("idCheck");
        $("#idCheck").click(function() {
          ...
          $.ajax({
            ...
            success : function(result) {
              if(result == "yes") {
                alert("아이디 사용 가능");
                useIDCheck = true;  // 아이디 중복체크 사용유무 확인
              }else {
                alert("아이디 사용 불가능");
                useIDCheck = false; // 아이디 중복체크 사용유무 확인
        ...
        $("#mailAuth").click(function() {
          ...
          $.ajax({    // public ResponseEntity<String> authSend(EmailDTO dto, HttpSession session)
            url : '/email/authcode', 
            type : 'get', // @GetMapping
        ...
        // 인증확인   <button type="button" class="btn btn-info" id="btnConfirmAuth">인증확인</button>
        $("#btnConfirmAuth").click(function() {
          ...
          $.ajax({
            ...
            type : 'get',
            dataType : 'text',  // success, fail, request
